...
def getSubjectsList(study_path):
...
class LitBasicModel(L.LightningModule):
    def __init__(self, model, learning_rate, train_batch_size, val_batch_size, test_batch_size):
        super().__init__()
    ...
    def training_step(self, train_batch, batch_idx):
        return self.__step(train_batch, batch_idx, self.train_batch_size, "train")
    
    def validation_step(self, val_batch, batch_idx):
        return self.__step(val_batch, batch_idx, self.val_batch_size, "val")
    
    def test_step(self, test_batch, batch_idx):
        return self.__step(test_batch, batch_idx, self.test_batch_size, "test")
...
class DiffusionMRIDataModule(L.LightningDataModule):
    def __init__(self, data_dir, batch_size, num_workers, moltiplicator) -> None:
        super().__init__()
        self.data_dir = data_dir
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.k = moltiplicator
    ...
    def setup(self, stage= None) -> None:
        ...
        self.testing_set =       tio.SubjectsDataset(testing_subjects*k,     transform=tio.Compose([preprocessing_transform]))
...
def splitFixTrainTestVal(subjects_list):
    test = [2, 16, 13, 12]
    val = [0, 7, 10, 14]
    train = set(range(19))
    ...
    return training_subjects, validation_subjects, testing_subjects

def basicModel():
    subjectsList = getSubjectsList("../../study")
    training_subjects, validation_subjects, testing_subjects = splitFixTrainTestVal(subjectsList)

    maxSize = findSizeCrop(subjectsList, "wfvf")
    preprocessing_transform = tio.Compose([
        ## Preprocessing ##
        # Spatial
        tio.transforms.ToCanonical(),
        tio.transforms.Resample("wfvf"),
        tio.transforms.CropOrPad((maxSize[0], maxSize[1], maxSize[2])),
        tio.transforms.CopyAffine("wfvf"),
    ...
    basicModel = LitBasicModel(model, 1e-3, train_batch_size, val_batch_size, test_batch_size)
    trainer = L.Trainer(
        accelerator="gpu",
        devices=[0],
        max_epochs=50,
        ...
        log_every_n_steps=val_batch_size
    )
    tuner = Tuner(trainer)
    tuner.scale_batch_size(basicModel, train_dataloaders=[training_loader], val_dataloaders=[validation_loader])

    trainer.fit(basicModel, training_loader, validation_loader)
